...
class ChangeFilePathHandler(object):
    ...
    def _change_file_uuid_map(self, repo_id, path, new_path, is_dir, src_repo_id = None):
        ...
        old_repo_id = src_repo_id if src_repo_id else repo_id
        old_path_md5 = self.md5_repo_id_parent_path(old_repo_id, old_dir)
        result = self.session.execute(text('select 1 from tags_fileuuidmap where repo_id_parent_path_md5=:path_md5 and filename=:filename and is_dir=:is_dir'),
                                 {'path_md5': old_path_md5, 'filename': old_file, 'is_dir': is_dir})
        ...
        if result.rowcount != 0:
            ...
            self.session.execute(text('update tags_fileuuidmap set repo_id=:new_repo_id, parent_path=:new_dir, filename=:new_file, '
                                 ...
                                 'repo_id_parent_path_md5=:old_md5 and filename=:filename and is_dir=:is_dir'),
                            {'new_repo_id': repo_id, 'new_dir': new_dir, 'new_file': new_file, 'new_md5': path_md5,
                             'old_md5': old_path_md5, 'filename': old_file, 'is_dir': is_dir})
            self.session.commit()
