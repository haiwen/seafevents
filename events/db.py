...
def get_last_file_history(session, repo_id, path, current_revision_id):
    repo_id_path_md5 = hashlib.md5((repo_id + path).encode('utf8')).hexdigest()
    current_item = session.scalars(select(FileHistory).where(FileHistory.repo_id_path_md5 == repo_id_path_md5).
                                   order_by(desc(FileHistory.id)).limit(1)).first()
    ...
    next_query_stmt = select(FileHistory).where(FileHistory.file_uuid == current_item.file_uuid, FileHistory.timestamp < event.timestamp).order_by(desc(FileHistory.timestamp)).limit(1)
