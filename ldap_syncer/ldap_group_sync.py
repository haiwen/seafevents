...
class LdapGroupSync(LdapSync):
    ...
    def get_data_from_ldap_by_server(self, config):
        ...
        if config.group_object_class.lower() == 'organizationalunit':
            grp_data_ldap = self.get_ou_data(ldap_conn, config)
        elif config.group_object_class == 'posixGroup':
    ...
    def get_common_group_data(self, ldap_conn, config):
        ...
        config.import_group_structure = False
        ...
        for base_dn in base_dns:
            if base_dn == '':
                continue
            results = None
            scope = SCOPE_ONELEVEL if config.import_group_structure else SCOPE_SUBTREE
    ...
    def get_group_member_from_ldap(self, config, ldap_conn, base_dn, grp_data, sort_list, parent_dn):
        ...
        if attrs.has_key(config.group_member_attr):
            ...
            if config.import_group_structure:
                return []
            else:
                return all_mails
        # user member
        ...
        for k, v in ldap_tups:
            if grp_dn_pairs.has_key(k):
                ...
                if super_user is None:
                    if v.config.import_group_structure:
                ...
                self.agroup += 1
                v.group_id = group_id
                if v.config.create_group_repo and v.config.import_group_structure:
