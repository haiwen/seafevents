...
from seaserv import get_ldap_groups, get_group_members, add_group_uuid_pair, \
        get_group_uuid_pairs, create_group, group_add_member, group_remove_member, \
...
class LdapGroup(object):
    def __init__(self, name, creator, members, parent_uuid=None, group_id=0, is_department=False):
        self.name = name
...
class LdapGroupSync(LdapSync):
    ...
    def get_group_member_from_ldap(self, config, ldap_conn, base_dn, grp_data, sort_list, parent_uuid):
        ...
        if type(attrs) != dict:
            return all_mails
        group_uuid = uuid_bytes_to_str(attrs[config.group_uuid_attr][0])

        if grp_data.has_key(group_uuid):
        ...
        for base_dn in base_dns:
            ...
            if config.use_page_result:
                results = ldap_conn.paged_search(base_dn, SCOPE_SUBTREE,
                                                search_filter,
                                                [config.group_member_attr, 'cn',
                                                 config.department_name_attr, config.group_uuid_attr])
            ...
            for result in results:
                ...
                if not attrs.has_key(config.group_member_attr):
                    ...
                    continue
                if grp_data_ldap.has_key(group_uuid):
                    continue
                all_mails = []
                for member in attrs[config.group_member_attr]:
                ...
                if config.sync_group_as_department:
                    ...
                    name = attrs['cn'][0]
                grp_data_ldap[group_uuid] = LdapGroup(name, None,
    ...
    def get_ou_data(self, ldap_conn, config):
        ...
        for base_dn in base_dns:
            ...
            result = ldap_conn.search(base_dn, SCOPE_BASE,
                                      search_filter,
                                      ['ou', config.department_name_attr, config.group_uuid_attr])
            ...
            self.get_ou_member (config, ldap_conn, base_dn, sort_list, name, group_uuid, None, grp_data_ou)
    ...
    def get_ou_member(self, config, ldap_conn, base_dn, sort_list, department_name, group_uuid, parent_uuid, grp_data_ou):
        if config.group_filter != '':
            search_filter = '(&(|(objectClass=organizationalUnit)(objectClass=%s))(%s))' % \
                             (config.user_object_class,
                              config.group_filter)
        ...
        # empty ou
        if not results:
            group = LdapGroup(department_name, None, [], parent_uuid, 0, True)
        ...
        for pair in results:
            ...
            if attrs.has_key('ou'):
                name = self.get_department_name (config, attrs, attrs['ou'][0])
                member_uuid = uuid_bytes_to_str(attrs[config.group_uuid_attr][0])
                self.get_ou_member (config, ldap_conn, member_dn,
                                    sort_list, name,
                                    member_uuid,
                                    group_uuid,
                                    grp_data_ou)

        group = LdapGroup(department_name, None, sorted(set(mails)), parent_uuid, 0, True)
        sort_list.append((group_uuid, group))
        grp_data_ou[group_uuid] = group
    ...
    def create_and_add_group_to_db(self, group_uuid, group, grp_uuid_pairs, grp_data_ldap):
        if group.is_department and grp_uuid_pairs.has_key(group_uuid):
        ...
        if not group.is_department:
            ...
            if not group.parent_uuid:
                parent_id = -1
            elif grp_uuid_pairs.has_key(group.parent_uuid):
        ...
        if group_id < 0:
            ...
            return
        ret = add_group_uuid_pair(group_id, group_uuid)
        if ret < 0:
    ...
    def sync_data(self, data_db, data_ldap):
        ...
        for uuid_pair in uuid_pairs:
            grp_uuid_pairs[uuid_pair.uuid] = uuid_pair.group_id
        # sync deleted group in ldap to db
        grp_uuid_pairs_tmp = grp_uuid_pairs.copy()
        for k in grp_uuid_pairs_tmp.iterkeys():
            if not data_ldap.has_key(k):
                deleted_group_id = grp_uuid_pairs_tmp[k]
                if (not data_db[deleted_group_id].is_department and self.settings.del_group_if_not_found) or \
                   (data_db[deleted_group_id].is_department and self.settings.del_department_if_not_found):
                    grp_uuid_pairs.pop(k)
                    ret = remove_group(deleted_group_id, '')
                    if ret < 0:
                        logger.warning('remove group %d failed.' % deleted_group_id)
                        continue
                    logger.debug('remove group %d success.' % deleted_group_id)
                    self.dgroup += 1

        # sync undeleted group in ldap to db
        super_user = None

        # ldap_tups = [('group_uuid', LdapGroup)...]
        ldap_tups = self.sort_list

        for k, v in ldap_tups:
            if grp_uuid_pairs.has_key(k):
                v.group_id = grp_uuid_pairs[k]
                # group data lost in db
                if not data_db.has_key(grp_uuid_pairs[k]):
                    continue
                ...
                self.create_and_add_group_to_db(k, v, grp_uuid_pairs, data_ldap)
