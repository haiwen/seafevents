...
class LdapGroupSync(LdapSync):
        ...
        for base_dn in base_dns:
            ...
            for result in results:
                ...
                if not isinstance(attrs, dict):
                    continue
                self.get_group_member_from_ldap(config, ldap_conn, group_dn, grp_data_ldap, sort_list, parent_dn=None, depth=1)
    ...
    def get_group_member_from_ldap(self, config, ldap_conn, base_dn, grp_data, sort_list, parent_dn, depth=1):
        """
        ...
        if config.group_member_attr in attrs and attrs[config.group_member_attr] != ['']:
            for member in attrs[config.group_member_attr]:
                mails = self.get_group_member_from_ldap(config, ldap_conn, member, grp_data, sort_list, base_dn, depth)
                if not mails:
                    continue
