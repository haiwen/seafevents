...
class VirusFile(Base):
    ...
    has_handle = Column(Boolean, nullable=False, index=True)
    has_ignore = Column(Boolean, nullable=False)
    __table_args__ = {'extend_existing':True}

    def __init__(self, repo_id, commit_id, file_path, has_handle, has_ignore):
        self.repo_id = repo_id
