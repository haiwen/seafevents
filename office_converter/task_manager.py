...
from seafevents import get_ttc_font_path
...
class Worker(threading.Thread):
    ...
    def _handle_task(self, task):
        """
                    libreoffice
        Excel  ===============>  html
                         libreoffice          if enabled
        Document file  ===============>  pdf  =======> pdf with watermark

            if enabled
        pdf  =======> pdf with watermark

        Error:
            E004: failed  to convert pdf
            E005: failed to add watermark
        """
